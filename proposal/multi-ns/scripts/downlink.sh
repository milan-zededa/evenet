...
#
# Usage: ./downlink.sh [<veth-name> <peer-namespace> <bridge-name>]...
...
ip link set ${veth} netns ${peer_ns}
ip link set ${veth}.1 up
ip netns exec ${peer_ns} ip link set ${veth} up
ip netns exec ${peer_ns} ip link set dev ${veth} master ${bridge_name}
