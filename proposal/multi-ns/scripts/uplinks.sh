...
ip link set keth2 netns ni4
ip link set gw2 up
ip netns exec ni4 ip link set keth2 up
ip addr add 192.168.2.1/24 dev gw2
ip netns exec ni4 ip link set keth2 up
ip netns exec ni4 ip link add name eth2 type bridge
ip netns exec ni4 ip link set dev eth2 up
ip netns exec ni4 ip link set dev keth2 master eth2
ip netns exec ni4 ip addr add 192.168.2.2/24 dev eth2
